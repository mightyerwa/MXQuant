    ...
    def __init__(self,
        ...
        if weight is not None:
            ...
            if group_size is not None:
                ...
                dim_in_feature = weight.shape[1] // group_size
                self.bound_factor = nn.Parameter(torch.ones((dim_out_feature, dim_in_feature, 1)) * 4)
            else:
    ...
    def fake_quant(self, x: torch.Tensor):
        ...
        if self.quant_weight:
            if self.group_size is not None:
                ori_shape = x.shape
                assert ori_shape[
                           -1] % self.group_size == 0, "hidden_size or in_feature of weight must be divisible by group_size"
